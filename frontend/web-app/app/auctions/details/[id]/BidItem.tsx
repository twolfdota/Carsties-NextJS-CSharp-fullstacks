...
export default function BidItem({ bid }: Props) {
    function getBidInfo() {
        ...
        switch (bid.bidStatus) {
            ...
            default:
                bgColor = "bg-red-200"
                text = "Bid was placed after auction finished"
                break;
        }
    ...
    return (
        <div className={`border-gray-300 border-2 px-3 py-2 rounded-lg 
        flex justify-between items-center mb-2 ${getBidInfo().bgColor}`}>
            <div className='flex flex-col'>
                <span>Bidder: {bid.bidder}</span>
                <span className='text-gray-700 text-sm'>Time: {format(bid.bidTime,"dd MMM yyyy h:mm:sss a")}</span>
            </div>
