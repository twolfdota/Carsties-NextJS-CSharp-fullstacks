...
export default function AuctionForm({ auction }: Props) {
  ...
  async function onSubmit(data: FieldValues) {
    try {
      ...
      toast.error(error.status || 404 + ' ' + error.message)
    }
  }
    ...
    <form className='flex flex-col mt-3' onSubmit={handleSubmit(onSubmit)}>
      ...
      {pathName === "/auctions/create" && (
        <>
          ...
          <div className='grid grid-cols-2 gap-3'>
            <Input name="reservePrice" label="Reserve price (enter 0 if no reserve)" type="number"
              control={control} rules={{ required: "Reserve price is required" }} />
            <DateInput name="auctionEnd" label="Auction end date/time" control={control}
